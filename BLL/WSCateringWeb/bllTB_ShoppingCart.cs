...
namespace CommunityBuy.BLL
{
    ...
    public class bllTB_ShoppingCart : bllBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string CheckPageInfo(string type,string BusCode, string CCname, string TStatus, string PKCode, string AutoDelTime, string IsAutoDelete, string MaxNum,string CCode)
        {
            ...
            //验证数据
            CheckValue<TB_ShoppingCartEntity>(EName, EValue, ref errorCode, new TB_ShoppingCartEntity());
            //特殊验证写在下面
            ...
            else//组合对象数据
            {
				...
				Entity.AutoDelTime = Helper.StringToDateTime(AutoDelTime);
				Entity.IsAutoDelete = IsAutoDelete;
				Entity.MaxNum = Helper.StringToInt(MaxNum);
            }
            return strRetuen;
        }
        ...
        /// 增加一条数据
        /// </summary>
        public DataTable Add(string GUID, string UID,string BusCode, string CCname, string TStatus, string PKCode, string AutoDelTime, string IsAutoDelete, string MaxNum,string CCode, operatelogEntity entity)
        {
			PKCode = "0";
            ...
            string strReturn = CheckPageInfo("add", BusCode, CCname, TStatus, PKCode, AutoDelTime, IsAutoDelete, MaxNum,CCode);
            //数据页面验证
        ...
        /// 更新一条数据
        /// </summary>
        public DataTable Update(string GUID, string UID,string BusCode, string CCname, string TStatus, string PKCode, string AutoDelTime, string IsAutoDelete, string MaxNum,string CCode, operatelogEntity entity)
        {
            ...
            dtBase.Clear();
            string spanids = string.Empty;
            string strReturn = CheckPageInfo("update", BusCode, CCname, TStatus, PKCode, AutoDelTime, IsAutoDelete, MaxNum,CCode);
            //数据页面验证
            if (!CheckControl(strReturn, spanids))
            {
                return dtBase;
			...
			//获取更新前的数据对象
            TB_ShoppingCartEntity OldEntity = new TB_ShoppingCartEntity();
            ...
            if (CheckResult(result))
            {
                ...
                if (entity != null)
                {
                    blllog.Add<TB_ShoppingCartEntity>(entity, Entity, OldEntity);
                }
            }
            return dtBase;
        }
        ...
        private TB_ShoppingCartEntity SetEntityInfo(DataRow dr)
        {
			...
			Entity.AutoDelTime = Helper.StringToDateTime(dr["AutoDelTime"].ToString());
			Entity.IsAutoDelete = dr["IsAutoDelete"].ToString();
			Entity.MaxNum = Helper.StringToInt(dr["MaxNum"].ToString());
            return Entity;
        }
    }
}