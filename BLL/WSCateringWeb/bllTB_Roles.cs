...
namespace CommunityBuy.BLL
{
    ...
    public class bllTB_Roles : bllBase
    {
        ...
        public string CheckPageInfo(string type, string Id, string BusCode, string StoCode, string CCname, string TStatus, string UCname, string SCope, string RoleName, string RoleParent, string RoleDescr, string RoleDisCount, string RoleEnable, string MaxDiffPrice, string MaxPrefePrice, string IsSig, string Sigcredit, string RoleType, string TerminalType, string CCode, string UCode)
        {
            ...
            //验证数据
            CheckValue<TB_RolesEntity>(EName, EValue, ref errorCode, new TB_RolesEntity());
            //特殊验证写在下面
            ...
            else//组合对象数据
            {
                ...
                Entity.SCope = Helper.StringToInt(SCope);
                Entity.RoleName = RoleName;
                Entity.RoleParent = Helper.StringToInt(RoleParent);
                Entity.RoleDescr = RoleDescr;
                Entity.RoleDisCount = RoleDisCount;
                Entity.RoleEnable = RoleEnable;
                Entity.MaxDiffPrice = Helper.StringToDecimal(MaxDiffPrice);
                Entity.MaxPrefePrice = Helper.StringToDecimal(MaxPrefePrice);
                Entity.IsSig = IsSig;
                Entity.Sigcredit = Helper.StringToDecimal(Sigcredit);
        ...
        /// 增加一条数据
        /// </summary>
        public DataTable Add(string GUID, string UID, out  string Id, string BusCode, string StoCode, string CCname, string TStatus, string UCname, string SCope, string RoleName, string RoleParent, string RoleDescr, string RoleDisCount, string RoleEnable, string MaxDiffPrice, string MaxPrefePrice, string IsSig, string Sigcredit, string RoleType, string TerminalType, string CCode, string UCode, string FunctionIds, operatelogEntity entity)
        {
            Id = "0";
            ...
            string strReturn = CheckPageInfo("add", Id, BusCode, StoCode, CCname, TStatus, UCname, SCope, RoleName, RoleParent, RoleDescr, RoleDisCount, RoleEnable, MaxDiffPrice, MaxPrefePrice, IsSig, Sigcredit, RoleType, TerminalType, CCode, UCode);
            //数据页面验证
            ...
            int result = dal.Add(ref Entity, FunList);
            //检测执行结果
            CheckResult(result);
            return dtBase;
        }
        ...
        /// 更新一条数据
        /// </summary>
        public DataTable Update(string GUID, string UID, string Id, string BusCode, string StoCode, string CCname, string TStatus, string UCname, string SCope, string RoleName, string RoleParent, string RoleDescr, string RoleDisCount, string RoleEnable, string MaxDiffPrice, string MaxPrefePrice, string IsSig, string Sigcredit, string RoleType, string TerminalType, string CCode, string UCode, string FunctionIds, operatelogEntity entity)
        {
            ...
            dtBase.Clear();
            string spanids = string.Empty;
            string strReturn = CheckPageInfo("update", Id, BusCode, StoCode, CCname, TStatus, UCname, SCope, RoleName, RoleParent, RoleDescr, RoleDisCount, RoleEnable, MaxDiffPrice, MaxPrefePrice, IsSig, Sigcredit, RoleType, TerminalType, CCode, UCode);
            //数据页面验证
            if (!CheckControl(strReturn))
            {
                return dtBase;
            ...
            //获取更新前的数据对象
            TB_RolesEntity OldEntity = new TB_RolesEntity();
            ...
            //更新数据
            int result = dal.Update(Entity, FunList);
            //检测执行结果
            if (CheckResult(result))
            {
                ...
                if (entity != null)
                {
                    blllog.Add<TB_RolesEntity>(entity, Entity, OldEntity);
                }
            }
            return dtBase;
        }
        ...
        private TB_RolesEntity SetEntityInfo(DataRow dr)
        {
            ...
            Entity.SCope = Helper.StringToInt(dr["SCope"].ToString());
            Entity.RoleName = dr["RoleName"].ToString();
            Entity.RoleParent = Helper.StringToInt(dr["RoleParent"].ToString());
            Entity.RoleDescr = dr["RoleDescr"].ToString();
            Entity.RoleDisCount = dr["RoleDisCount"].ToString();
            Entity.RoleEnable = dr["RoleEnable"].ToString();
            Entity.MaxDiffPrice = Helper.StringToDecimal(dr["MaxDiffPrice"].ToString());
            Entity.MaxPrefePrice = Helper.StringToDecimal(dr["MaxPrefePrice"].ToString());
            Entity.IsSig = dr["IsSig"].ToString();
            Entity.Sigcredit = Helper.StringToDecimal(dr["Sigcredit"].ToString());
