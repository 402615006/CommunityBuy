    ...
    public class blloperatelog
    {
        ...
        public int Add<T>(operatelogEntity Entity, T NewEntity, T OldEntity)
        {
            ...
            if (PropertyList.Length > 0)
            {
                ...
                foreach (PropertyInfo item in PropertyList)
                {
                    try
                    {
                        ...
                        if (NewEntity.GetType().GetProperty(item.Name) != null && OldEntity.GetType().GetProperty(item.Name) != null)
                        {
            ...
            if (sb.Length > 0)
            {
                ...
                Entity.buscode = WebCache.GetCache(Entity.cuser.ToString() + "_buscode").ToString();
