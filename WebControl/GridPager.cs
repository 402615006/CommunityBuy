...
namespace CommunityBuy.WebControl
{
    ...
    public class GridPager : Panel, INamingContainer, IPostBackEventHandler, IPostBackDataHandler
    {
        ...
        [Browsable(true),
        DefaultValue(null),
        Category("文本框设置获取Grid分页"),
        Description("设定Grid的分页")]
        public string TextPageSize
        {
            get
            {
                return (string)ViewState["TextPageSize"];
            }
            set
            {
                ViewState["TextPageSize"] = value;
            }
        }
       ...
       Category("文本框及提交按钮"),
       Description("页索引输入文本框后的文本内容字符串")]
        public string PageSizeSetButton
        {
            get
            {
                return (string)ViewState["PageSizeSetButton"];
            }
            set
            {
                ViewState["PageSizeSetButton"] = value;
            }
        }
        ...
        public string SetButtonText
        {
            get
            {
                object obj = ViewState["SetButtonText"];
                return (obj == null) ? "set" : (string)obj;
            }
            set
            {
                if (value.Length > 0)
                    ViewState["SetButtonText"] = value;
            }
        }
        ...
        public string CustomInfoText
        {
            get
            {
                ...
                sb.Append("&nbsp;&nbsp;记录总数：<font color=\"blue\"><b>" + this.RecordCount.ToString() + "</b></font>&nbsp;&nbsp;");
                sb.Append(" 总页数：<font color=\"blue\"><b><span id=\"pageCount\" class=\"spanPageCount\">" + this.PageCount.ToString() + "</span></b></font>&nbsp;&nbsp;");
                sb.Append(" 当前页：<font color=\"red\"><b>" + this.CurrentPageIndex.ToString() + "</b></font>&nbsp;&nbsp;");
                return sb.ToString();
            }
        }
       ...
       DefaultValue("divPager")]
        ...
        DefaultValue("分页设置错误！"),
        Category("Data")]
        public string PageSizeOutOfRangeErrorString
        {
            get
            {
                object obj = ViewState["PageSizeOutOfRangeErrorString"];
                return (obj == null) ? "分页设置错误！" : (string)obj;
            }
            set
            {
                ViewState["PageSizeOutOfRangeErrorString"] = value;
            }
        }
            ...
            if (Page.IsPostBack)
            {
                if (urlPaging)
                {
                    ...
                    try
                    {
                        ...
                        string isPagesize = Page.Request.Form[this.UniqueID + "_TextPageSize_inputsize"];
                        if (!string.IsNullOrEmpty(isPagesize))
                        {
                            PageSize = int.Parse(isPagesize);
                            //Page.Response.Redirect(currentUrl + "&PageSize=" + isPagesize);
                            OnPageSizeChanged(new PageSizeChangedEventArgs(PageSize));
                        }
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(inputPageIndex))
                    {
                        if (int.TryParse(inputPageIndex, out IsInt))
                        {
                            CurrentPageIndex = int.Parse(inputPageIndex);
                        }
                    }
                    inputPagesize = Page.Request.Form[this.UniqueID + "_TextPageSize_inputsize"];
                    if (int.TryParse(inputPagesize, out IsInt))
                    {
                        if (!string.IsNullOrEmpty(inputPagesize))
                        {
                            PageSize = int.Parse(inputPagesize);
                            OnPageSizeChanged(new PageSizeChangedEventArgs(PageSize));
                        }
                    }
                    else
        ...
        {
            string checkPageSizescript = "<script language=\"Javascript\">function doCheckPageSize(el){var r=new RegExp(\"^\\\\s*(\\\\d+)\\\\s*$\");if(r.test(el.value)){if(RegExp.$1<1){ScriptMessBox('" + PageSizeOutOfRangeErrorString + "','信息提示',300,200);document.all[\'" + this.UniqueID + "_TextPageSize_inputsize\'].select();return false;}return true;}ScriptMessBox('" + PageSizeOutOfRangeErrorString + "','信息提示',300,200);document.all[\'" + this.UniqueID + "_TextPageSize_inputsize\'].select();return false;}</script>";
            if (!Page.ClientScript.IsClientScriptBlockRegistered("checkpagesizeinput"))
            {
                Page.ClientScript.RegisterClientScriptBlock(typeof(Page), "checkpagesizeinput", checkPageSizescript);
            }
            string checkscript = "<script language=\"Javascript\">function doCheck(el){var r=new RegExp(\"^\\\\s*(\\\\d+)\\\\s*$\");if(r.test(el.value)){if(RegExp.$1<1||RegExp.$1>$(\"#pageCount\").html()){ ScriptMessBox('" + PageIndexOutOfRangeErrorString + "','信息提示',300,200);document.all[\'" + this.UniqueID + "_input\'].select();return false;}return true;}ScriptMessBox('" + InvalidPageIndexErrorString + "','信息提示',300,200);document.all[\'" + this.UniqueID + "_input\'].select();return false;}</script>";
        ...
        public override void RenderBeginTag(HtmlTextWriter writer)
        {
            ...
            if ((ShowCustomInfoSection == ShowCustomInfoSection.Left || ShowCustomInfoSection == ShowCustomInfoSection.Right) && showPager)
            {
                ...
                writer.AddAttribute(HtmlTextWriterAttribute.Class, "pageTable");
        ...
        protected override void RenderContents(HtmlTextWriter writer)
        {
            ...
            if (ShowCustomInfoSection == ShowCustomInfoSection.Left)
            {
                ...
                writer.AddAttribute(HtmlTextWriterAttribute.Name, this.UniqueID + "_TextPageSize_inputsize");
                ...
                writer.RenderEndTag();
                writer.Write("&nbsp;条/页");
                if (PageSizeSetButton != null)
                    writer.Write(PageSizeSetButton);
                writer.AddAttribute(HtmlTextWriterAttribute.Type, "submit");
                writer.AddAttribute(HtmlTextWriterAttribute.Name, "TextPageButton_input");
                writer.AddAttribute(HtmlTextWriterAttribute.Class, "buttonset");
                writer.AddAttribute(HtmlTextWriterAttribute.Title, "设置记录数");
                //writer.AddAttribute(HtmlTextWriterAttribute.Value, SetButtonText);
                ...
                string scriptRef = "doCheckPageSize(document.all[\'" + this.UniqueID + "_TextPageSize_inputsize\'])";
                ////string clickScript = "if(" + scriptRef + "){return false;}"; "return " + scriptRef;
                //writer.AddAttribute(HtmlTextWriterAttribute.Onclick, Page.GetPostBackClientEvent(this, "pagesize"));
                writer.AddAttribute(HtmlTextWriterAttribute.Onclick, scriptRef);
                writer.RenderBeginTag(HtmlTextWriterTag.Input);
                writer.RenderEndTag();
            ...
            if ((ShowInputBox == ShowInputBox.Always) || (ShowInputBox == ShowInputBox.Auto && PageCount >= ShowBoxThreshold))
            {
                ...
                writer.AddAttribute(HtmlTextWriterAttribute.Class, "buttongoto");                
                writer.AddAttribute(HtmlTextWriterAttribute.Title, "跳转到指定页");
        ...
        #region PageSizeChanged Event

        public event PageSizeChangedEventHandler PageSizeChanged;
        #endregion

        #region OnPageSizeChanged Method

        /// <summary>
        /// 引发 <see cref="PageSizeChanged"/> 事件。这使您可以为事件提供自定义处理程序。
        /// </summary>
        /// <param name="e">一个 <see cref="PageChangedEventArgs"/>，它包含事件数据。</param>
    ...
    #region PageSizeChangeEventHandler
    /// <summary>
    /// 表示处理 PageSizeChanged事件的方法。
    /// </summary>
    public delegate void PageSizeChangedEventHandler(object src, PageSizeChangedEventArgs e);
    #endregion
    ...
    public sealed class PageSizeChangedEventArgs : EventArgs
    {
