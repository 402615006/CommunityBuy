...
namespace CommunityBuy.IServices
{
    ...
    /// 退单原因管理接口类
    /// </summary>
    public class WS_TB_BackReason : ServiceBase
    {
        bllTB_BackReason bll = new bllTB_BackReason();
        ...
        {
            //要检测的参数信息
            List<string> pra = new List<string>() { "GUID", "USER_ID", "BusCode", "StoCode", "CCname", "UCname", "TStatus", "Sort", "PKCode", "Reason", "Ascription", "Remark", "CCode", "UCode" };
            //检测方法需要的参数
            if (!CheckActionParameters(dicPar, pra))
            {
                return;
            ...
            string Reason = dicPar["Reason"].ToString();
            string Ascription = dicPar["Ascription"].ToString();
            string Remark = dicPar["Remark"].ToString();
            string CCode = dicPar["CCode"].ToString();
            string UCode = dicPar["UCode"].ToString();
            //调用逻辑
            logentity.pageurl = "TB_BackReasonEdit.html";
            logentity.logcontent = "新增退单原因管理信息";
            ...
            dt = bll.Add(GUID, USER_ID, BusCode, StoCode, CCname, UCname, TStatus, Sort, PKCode, Reason, Ascription, Remark, CCode, UCode, logentity);
        ...
        private void Update(Dictionary<string, object> dicPar)
        {
            ...
            //调用逻辑
            logentity.pageurl = "TB_BackReasonEdit.html";
            logentity.logcontent = "修改id为:" + PKCode + "的退单原因管理信息";
            logentity.cuser = Helper.StringToLong(UCode);
            logentity.otype = SystemEnum.LogOperateType.Edit;
            logentity.buscode = GetCacheToUserBusCode(logentity.cuser.ToString());
            dt = bll.Update(GUID, USER_ID, BusCode, StoCode, CCname, UCname, TStatus, Sort, PKCode, Reason, Ascription, Remark, CCode, UCode, logentity);
        ...
        private void Delete(Dictionary<string, object> dicPar)
        {
            ...
            //调用逻辑
            logentity.pageurl = "TB_BackReasonList.html";
            logentity.logcontent = "删除id为:" + id + "的退单原因管理信息";
            logentity.cuser = Helper.StringToLong(USER_ID);
        ...
        private void UpdateStatus(Dictionary<string, object> dicPar)
        {
            ...
            logentity.pageurl = "TB_BackReasonList.html";
            logentity.logcontent = "修改状态id为:" + PKCode + "的退单原因管理信息";
            logentity.cuser = Helper.StringToLong(USER_ID);
