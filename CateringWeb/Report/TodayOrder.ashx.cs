...
namespace CommunityBuy.IServices
{
    ...
    /// </summary>
    public class TodayOrder : ServiceBase
    {
        ...
        public override void ProcessRequest(HttpContext context)
        {
            if (CheckParameters(context))//检测是否合法
            {
                ...
                if (dicPar != null)
                {
                    ...
                    {
                        case "gettodayorder"://列表
                            GetTodayOrder(dicPar);
                            break;
                    }
                }
            }
        ...
        public void GetTodayOrder(Dictionary<string, object> dicPar)
        {
            //判断时间
            ...
            DataSet ds = new BLL.bllPaging().GetDatasetByProcedure("[dbo].[p_TodayOrder]", ps);
            ...
            if (ds != null && ds.Tables.Count == 1)
            {
                ...
                if(dt_order!=null && dt_order.Rows.Count>0)
                {
                    ...
                    foreach (DataRow dr in dt_order.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(dr["BillCode"].ToString()))
                        {
                            newdr["field2_url"] = @"./TBBillingDetails.html?StoCode=" + stocode + "&BillCode=" + dr["BillCode"].ToString() + "&StoName=" + dr["StoName"].ToString();
                        ...
                        newdr["field4"] = dr["FTime"].ToString();
                        newdr["field5"] = dr["PayMoney"].ToString();
