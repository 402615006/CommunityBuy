...
namespace CommunityBuy.DAL
{
    ...
    public partial class dalTB_UserRole
    {
        ...
        public int Add(ref TB_UserRoleEntity Entity)
        {
            ...
            foreach (TB_UserSigSchemeEntity uss in Entity.UserSig)
            {
                sql.Append("insert into TB_UserSigScheme(BusCode,StoCode,CCode,CCname,CTime,SigCode,DayMoney,UserCode) values(@BusCode,(select stocode from TM_SignatureScheme where PKCode='"+uss.SigCode+"'),'" + uss.CCode + "','" + uss.CCname + "',getdate(),'" + uss.SigCode + "'," + uss.DayMoney + ",@UserId);");
            }
        ...
        public int Update(TB_UserRoleEntity Entity)
        {
            ...
            foreach (TB_UserSigSchemeEntity uss in Entity.UserSig)
            {
                sql.Append("insert into TB_UserSigScheme(BusCode,StoCode,CCode,CCname,CTime,SigCode,DayMoney,UserCode) values('" + uss.BusCode + "','" + uss.StoCode + "','" + uss.CCode + "','" + uss.CCname + "',getdate(),'" + uss.SigCode + "'," + uss.DayMoney + ",@UserId);");
            }
            sql.Append(" IF(@@ERROR=0)  BEGIN COMMIT TRAN tran1;set @returnval=0;END ELSE BEGIN ROLLBACK TRAN tran1; set @returnval=1; END  ");
            sql.Append(" END");
            sql.Append(" else ");
            ...
            foreach (TB_UserSigSchemeEntity uss in Entity.UserSig)
            {
                sql.Append("insert into TB_UserSigScheme(BusCode,StoCode,CCode,CCname,CTime,SigCode,DayMoney,UserCode) values(@BusCode,@StoCode,'" + uss.CCode + "','" + uss.CCname + "',getdate(),'" + uss.SigCode + "'," + uss.DayMoney + ",@UserId);");
            }
            sql.Append(" IF(@@ERROR=0)  BEGIN COMMIT TRAN tran1;set @returnval=0;END ELSE BEGIN ROLLBACK TRAN tran1; set @returnval=1; END  ");
            sql.Append(" END ");
            sql.Append(" select @returnval;");
