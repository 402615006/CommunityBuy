...
function freezeTable(table, freezeRowNum, freezeColumnNum, width, height) {
    ...
    divTableData.css({ "overflow": "auto", "width": width, "height": height, "position": "absolute", "z-index": "39" });
...
var reportid;
var reporttitleW;
function freeTable(id, titleW) {
    reportid = id;
    reporttitleW = titleW;
    var table = $("#" + id);                                //获取当前table
    var tableId = id;                        //table的ID之后作为参数传递    
    ...
    if (typeof (freezeRowNum) != 'undefined' || typeof (freezeColumnNum) != 'undefined') {
        ...
        freezeTable(table, freezeRowNum || 0, freezeColumnNum || 0, w, pageHeight() - titleW);
        $(window).resize(function () {
            setTimeout(function () {
                ...
                adjustTableSize(reportid, w, pageHeight() - reporttitleW);
            }, 50);
        });
    }
}

var width = $(".operbtn").width();
if (width <= 920) {
    if ($(".operbtn a").length > 9) {
        $(".righttable").css("margin-top", "80px");
    }
} else {
    $(".righttable").css("margin-top", "40px");
}
