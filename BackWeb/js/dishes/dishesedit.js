...
if ($("#stocodes").val() == "" || $("#stocodes").val() == undefined || $("#stocodes").val()==null) {
    $("#stocodes").val(stocode);
}
else {
    stocode = $("#stocodes").val();
}
                    ...
                    $(objdata.DishesMates).each(function (i, o) {
                        ...
                        if (o.StockMateUnits != undefined && o.StockMateUnits.length > 0) {
                            ...
                            $(o.StockMateUnits).each(function (j, q) {
                                ...
                                UnitInfo += '<option value="' + q.sjcb + '|' + q.matunitcode + '">' + q.matunitname + '</option>';
                        ...
                        row += '<td><input id="jlv" class="num empty" type="text" maxlength="10" style="width:70px;" value="' + jlv.toFixed(2) + '"  data-notempty="true" data-reg="Decimal" oninput="jlvinput(this,' + jlnum + ',' + i + ',' + xprice+')">%</td>';
                        row += '<td><input id="mlnum'+i+'" class="num empty" type="text" maxlength="10" style="width:70px;" value="' + mlnum + '"  data-notempty="true" data-reg="Decimal"></td>';
                        ...
                        row += '<td><img id="matadelete" alt="" onclick="TableInfoDelete(this,\'3\');" src="../images/btn_delete.png" class="list_button_delete" /></td></tr>';
...
function jlvinput(o, n, i, m) {
    var mlnum = parseFloat(($(o).val() / 100) * n).toFixed(2);
    $("#mlnum" + i).val(mlnum);
    $("#money" + i).html((mlnum * parseFloat(m)).toFixed(2));
}
...
function DishesMateAdd() {
    //选择原料
    ShowOpenpage('', 'DishesMaterefer.aspx?type=dishes&fun=bindDishesMate&stocode=' + $("#stocode").val() + '', '80%', '550px', false);
}
    ...
    if (ArrDishesMate != undefined) {
        for (var i = 0; i < ArrDishesMate.length; i++) {
            ...
            if (val.indexOf(',' + obj.matcode + ',') < 0) {
                ...
                row += '<td><input id="jlv" class="num empty" type="text" maxlength="10" style="width:70px;" value="0"  data-notempty="true" data-reg="Decimal"  oninput="jlvinput(this,' + jlnum + ',' + i + ')"></td>';
                row += '<td><input id="mlnum'+i+'" class="num empty" type="text" maxlength="10" style="width:70px;" value="0"  data-notempty="true" data-reg="Decimal"></td>';
